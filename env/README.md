...
Secrets for `ores-otel/ores-lib-core` are **committed, encrypted**, with [sops] + [age],
...
- **No multi-line values.** A PEM must be a single line with `\n` escapes;
  use a non-secret placeholder in documentation, for example
  `JWT_PRIVATE_KEY="<PEM contents with \\n escapes>"`.
...
```sh
just env-docker-run dev ghcr.io/ores-otel/ores-lib-core:dev
