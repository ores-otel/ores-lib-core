...
pub const SENSITIVE_FIELDS: &[&str] = &[
    "authorization", "cookie", "password", "secret", "token", "email", "normalized_email", "email_lookup_hmac", "access_token",
...
pub fn normalize_email_for_revocation(value: &str) -> Result<String, EmailNormalizationError> {
    ...
    if trimmed.is_empty() { return Err(EmailNormalizationError::Empty); }
    if trimmed.len() > 320 { return Err(EmailNormalizationError::TooLong); }
    if !trimmed.is_ascii() { return Err(EmailNormalizationError::NonAscii); }
...
fn valid_email_local_byte(value: u8) -> bool {
    value.is_ascii_alphanumeric()
        || matches!(value, b'.' | b'!' | b'#' | b'$' | b'%' | b'&' | b'\'' | b'*' | b'+' | b'-' | b'/' | b'=' | b'?' | b'^' | b'_' | b'`' | b'{' | b'|' | b'}' | b'~')
}
...
fn valid_domain_label(label: &str) -> bool {
    !label.is_empty()
        ...
        && !label.starts_with('-')
        && !label.ends_with('-')
        && label.bytes().all(|value| value.is_ascii_alphanumeric() || value == b'-')
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum IdempotencyDisposition { New, Replay, Conflict }
...
pub fn classify_idempotency(
    ...
    match existing_request_digest {
        ...
        Some(existing) if constant_time_eq(existing, incoming_request_digest) => IdempotencyDisposition::Replay,
...
mod tests {
    ...
    fn email_normalization_is_portable_and_strict() {
        assert_eq!(normalize_email_for_revocation("  Alex+Ops@Example.COM\n"), Ok("alex+ops@example.com".into()));
        assert_eq!(normalize_email_for_revocation("a..b@example.com"), Err(EmailNormalizationError::InvalidLocalPart));
        assert_eq!(normalize_email_for_revocation("alex@localhost"), Err(EmailNormalizationError::InvalidDomain));
        assert_eq!(redact_value("normalizedEmail", "alex@example.com"), REDACTED);
    }
    #[test]
    fn organization_authorization_is_an_intersection() {
        let grants = vec![
            RevocationGrant { organization_id: "org-b".into(), sessions_revoke: true },
            RevocationGrant { organization_id: "org-a".into(), sessions_revoke: false },
            RevocationGrant { organization_id: "org-c".into(), sessions_revoke: true },
        ];
        let requested = vec!["org-a".into(), "org-b".into(), "org-unknown".into()];
        assert_eq!(authorized_organizations(Some(&requested), &grants), vec!["org-b".to_string()]);
    }
    #[test]
    fn idempotency_replays_only_an_identical_digest() {
        assert_eq!(classify_idempotency(None, &[1; 32]), IdempotencyDisposition::New);
        assert_eq!(classify_idempotency(Some(&[1; 32]), &[1; 32]), IdempotencyDisposition::Replay);
        assert_eq!(classify_idempotency(Some(&[1; 32]), &[2; 32]), IdempotencyDisposition::Conflict);
    }
}
